...
path = os.path.join(my_path, "../secrets/pusher_keys.json")
with open(path) as f:
    data = json.load(f)

# instantiate pusher
pusher = Pusher(app_id=u'data["pusher"]["app_id"]', key=u'data["pusher"]["key"]', secret=u'data["pusher"]["secret"]',
                cluster=u'data["pusher"]["cluster"]')
